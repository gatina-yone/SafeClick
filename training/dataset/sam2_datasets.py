...
class TorchTrainMixedDataset:
    def __init__(
        ...
        """
        参数：
            datasets（数据集列表）：要混合的数据集列表。
            batch_sizes（整数列表）：列表中每个数据集的批量大小。
            num_workers（整数）：每个数据加载器的工作线程数量。
            shuffle（布尔值）：是否打乱数据。
            pin_memory（布尔值）：如果为真，从磁盘加载张量时使用固定内存（页锁定内存）。
            drop_last（布尔值）：是否丢弃最后一批数据。
            collate_fn（可调用对象）：将一组样本合并成一个小批量的函数。
            worker_init_fn（可调用对象）：用于初始化每个数据加载器工作线程的函数。
            phases_per_epoch（整数）：每个训练周期的阶段数量。
            dataset_prob（浮点数列表）：选择从哪个数据加载器进行采样的概率。其总和应等于 1.0。
        """        
