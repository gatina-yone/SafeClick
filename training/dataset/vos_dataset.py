    ...
    def _get_datapoint(self, idx):
        ...
        for retry in range(MAX_RETRIES):
            try:
                ...
                # print("video, segment_loader = self.video_dataset.get_video()",idx)
                ...
                #sampled_frms_and_objs 从video里返回num_frames: 8帧信息
                #sampled_frms_and_objs.frames = SampledFramesAndObjects(frames=[VOSFrame(frame_idx=5, image_path='/data/sam2/sam2_example_20G
                # /train/JPEGImages/00501424/00005.jpg', data=None, is_conditioning_only=False), 
                #sampled_frms_and_objs.object_ids = object_ids=[np.uint8(5), np.uint8(2), np.uint8(1)])
        ...
        # datapoint = VideoDatapoint(frames=images,video_id=video.video_id,size=(h, w),)
        # frames [Frame(data=<PIL.Image.Image image mode=RGB size=640x332 at 0x7FA8C18A45C0>, objects=[Object(object_id=np.uint8(5), frame_index=5, segment=tensor([[0, 0, 0,  ..., 0, 0, 0],
        # datapoint.frames[0].objects[0].segment 表示第一帧的第一个物体的掩码np.unique=0,1
        # datapoint.video_id = 1   datapoint.size = (332, 640)

        for transform in self._transforms:
            datapoint = transform(datapoint, epoch=self.curr_epoch)
        # if(idx==2 or idx==1):
        #     breakpoint()
        #datapoint.frames[0].objects[0].segment.shape = torch.Size([1024, 1024]) 表示第一帧的第一个物体的掩码np.unique=0,1
        #datapoint.frames[0].data.shape = torch.Size([3, 1024, 1024])
        # breakpoint()
