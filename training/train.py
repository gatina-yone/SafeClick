...
import sys
sys.path.append('.')  # 将当前目录（包含training文件夹）添加到Python路径
...
def filelist_write(gt_folder,file_list_txt,num_frames):
    open (file_list_txt,'w').close()
    for file in os.listdir(gt_folder):
        if len(os.listdir(os.path.join(gt_folder,file)))>=num_frames:
            with open (file_list_txt,'a') as f:
                f.write(file+'\n')     


def main(args) -> None:
    # os.environ["HYDRA_FULL_ERROR"] = args.gpu_id
    cfg = compose(config_name=args.config)
    if cfg.launcher.experiment_log_dir is None:
        cfg.launcher.experiment_log_dir = os.path.join(
            os.getcwd(), "sam2_logs", args.config
        )
    sam2result_dir = args.savedir
    cfg.launcher.experiment_log_dir = (
        os.path.join(sam2result_dir,args.checkpoint,args.dataset) if args.dataset is not None else cfg.launcher.experiment_log_dir
    )
    cfg.dataset.img_folder = (
        cfg.dataset.img_folder.replace("mydataset",args.dataset) if args.dataset is not None else cfg.launcher.experiment_log_dir
    )
    cfg.dataset.gt_folder = (
        cfg.dataset.gt_folder.replace("mydataset",args.dataset) if args.dataset is not None else cfg.launcher.experiment_log_dir
    )
    cfg.dataset.file_list_txt = (
        cfg.dataset.file_list_txt.replace("mydataset",args.dataset) if args.dataset is not None else cfg.launcher.experiment_log_dir
    )
    cfg.scratch.num_epochs = (
        args.num_epochs if args.num_epochs is not None else cfg.scratch.num_epochs
    )
    # cfg.trainer.checkpoint.model_weight_initializer.state_dict.checkpoint_path
    cfg.trainer._target_ = (
        args.trainer if args.trainer is not None else cfg.trainer._target_
    )
    cfg.trainer.checkpoint.save_freq = (
        args.save_freq if args.save_freq is not None else cfg.trainer.checkpoint.save_freq
    )

    # breakpoint()
    filelist_write(cfg.dataset.gt_folder,cfg.dataset.file_list_txt,cfg.scratch.num_frames)
    # breakpoint()
    ...
    if submitit_conf.use_cluster:
        ...
        #(submitit_conf.port_range[0], submitit_conf.port_range[1]) = (10000, 65000)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-save_dir","--checkpoint",required=True, type=str, default='')
    parser.add_argument("--savedir", type=str, default="/data/sam2/sam2_M/")
