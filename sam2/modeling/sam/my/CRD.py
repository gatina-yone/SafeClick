...
class Feature_Reweighting(nn.Module):
    ...
    def forward(self, sup_fea, que_fea):
        ...
        # breakpoint()
        self_atten = torch.max(self_atten, -1, keepdim=True)[0].expand_as(self_atten) - self_atten  
...
if __name__ == '__main__':
    rga_module = Feature_Reweighting() # in_spatial= H * W
    input_tensor = torch.randn(40, 64, 64, 32)
    input_tensor = torch.randn(20, 64, 64, 32)
    print("Input shape:", input_tensor.shape)#Input shape: torch.Size([4, 64, 32, 32])
    output,alpha = rga_module(input_tensor,input_tensor)
    print("Output shape:", output.shape,alpha)#Input shape: torch.Size([4, 64, 32, 32])